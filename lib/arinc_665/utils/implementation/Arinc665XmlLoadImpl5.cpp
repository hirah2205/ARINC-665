...
Arinc665XmlLoadImpl5::Arinc665XmlLoadImpl5(
...
OptionalMediumNumber Arinc665XmlLoadImpl5::mediumNumber(
...
void Arinc665XmlLoadImpl5::mediaSet( const xmlpp::Element &mediaSetElement )
{
    ...
    nullptr != filesUserDefinedDataNode )
  {
    const auto userDefinedData{
      filesUserDefinedDataNode->get_first_child_text()->get_content() };
  ...
  {
    const auto userDefinedData{
      loadsUserDefinedDataNode->get_first_child_text()->get_content() };
  ...
  {
    const auto userDefinedData{
      batchesUserDefinedDataNode->get_first_child_text()->get_content() };
...
void Arinc665XmlLoadImpl5::regularFile(
...
void Arinc665XmlLoadImpl5::load(
  ...
  Media::ContainerEntity &parent )
{
  ...
  {
    const auto userDefinedData{
      userDefinedDataElement->get_first_child_text()->get_content() };
...
void Arinc665XmlLoadImpl5::loadDeferred(
  const xmlpp::Element &loadElement,
  Media::Load &load )
{
  ...
  const Media::ContainerEntity &parent ) const
{
  ...
  for ( auto const * const fileNode :
    loadElement.get_children( std::string{ fileElementsName } ) )
  {
    if ( nullptr == fileNode )
    {
      // Should never occur
...
void Arinc665XmlLoadImpl5::batch(
...
void Arinc665XmlLoadImpl5::loadBatchDeferred(
  const xmlpp::Element &batchElement,
  Media::Batch &batch )
{
    ...
    batchElement.get_children( "Target" ) )
  {
    ...
    for ( auto const * const loadNode : targetNode->get_children( "Load" ) )
    {
      ...
      auto load{ batch.parent()->load(
        std::filesystem::path{ loadFilePath, std::locale{} } ) };
      if ( !load )
      {
        BOOST_THROW_EXCEPTION( Arinc665Exception()
...
void Arinc665XmlLoadImpl5::baseFile(
  const xmlpp::Element &fileElement,
  ...
  std::string_view attribute )
{
  if (
    const auto checkValueString{
      element.get_attribute_value( std::string{ attribute } ) };
...
Arinc665XmlLoadImpl5::EntryType Arinc665XmlLoadImpl5::entryType(
  const xmlpp::Element &element )
{
